...
fn boolsToInt(comptime n: usize, vec: @Vector(n, bool)) u64 {
    comptime std.debug.assert(n <= 64);
    comptime var u = 0;
    var res: u64 = 0;
    inline while (u < n) {
        res = res | (@as(u64, @intFromBool(vec[u])) << u);
        u += 1;
    }
    return res;
}

fn intToBools(comptime n: usize, x: u64) @Vector(n, bool) {
    comptime std.debug.assert(n <= 64);
    comptime var u = 0;
    var res: @Vector(n, bool) = undefined;
    inline while (u < n) {
        res[u] = ((x >> u) & 1) == 1;
        u += 1;
    }
...
pub const Parser = struct {
    ...
    pub fn next(self: *Parser) ?Field {
        ...
        while (self._state.field_separators == 0 and self.hasNextChunk()) {
            ...
            var len: u8 = 0;
            var chunk: @Vector(chunk_size, u8) = @splat(@as(u8, 0));
            for (sub_text[0..extract], 0..) |c, i| {
                chunk[i] = c;
                len += 1;
            }
            ...
            const match_quotes: u64 = boolsToInt(chunk_size, chunk == quotes);

            const commas: @Vector(chunk_size, u8) = @splat(@as(u8, ','));
            const match_commas = boolsToInt(chunk_size, chunk == commas);

            const crs: @Vector(chunk_size, u8) = @splat(@as(u8, '\r'));
            const cr_matches = boolsToInt(chunk_size, chunk == crs);
            defer self._state.prev_cr = cr_matches;

            const lfs: @Vector(chunk_size, u8) = @splat(@as(u8, '\n'));
            const lf_matches = boolsToInt(chunk_size, chunk == lfs);
