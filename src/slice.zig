...
const ParserState = struct {
    prev_quote: ChunkBits = 0,
    prev_cr: ChunkBits = 0,
    prev_quote_ends: ChunkBits = 0,
    prev_field_seps: ChunkBits = 1 << (chunk_size - 1),
    field_separators: ChunkBits = 0,
    start_chunk: u64 = 0,
...
pub const Parser = struct {
    ...
    fn startPos(self: *const Parser) u64 {
        return self._state.start_chunk * chunk_size + self._state.start_chunk_pos;
    }
    ...
    fn match(ch: u8, slice: []const u8) ChunkBits {
        var res: ChunkBits = 0;
        for (slice, 0..) |c, i| {
            res |= @as(ChunkBits, @intFromBool(c == ch)) << @truncate(i);
        }
        return res;
    }
    ...
    pub fn next(self: *Parser) ?Field {
        ...
        while (self._state.field_separators == 0 and self.hasNextChunk()) {
            ...
            const sub_text = self._text[(self._state.next_chunk * chunk_size)..];
            const extract = @min(chunk_size, sub_text.len);
            const chunk = sub_text[0..extract];

            const match_quotes: ChunkBits = match('"', chunk);
            ...
            if (len < chunk_size) {
                lf_matches |= @as(ChunkBits, 1) << @truncate(len);
            }

            const carry: ChunkBits = @bitCast(-%@as(
                i64,
        ...
        self._state.field_separators ^= @as(ChunkBits, 1) << @truncate(chunk_end);
        if (end_pos < self._text.len and self._text[end_pos] == '\r') {
            self._state.field_separators ^= @as(ChunkBits, 1) << @truncate(chunk_end + 1);
    ...
    fn quotedRegions(m: ChunkBits) ChunkBits {
        var x: ChunkBits = m;
        var res: ChunkBits = x;
        while (x != 0) {
            const x1: ChunkBits = @bitCast(-%@as(i64, @bitCast(x)));
            res = res ^ (x1 ^ x);
            x = x & x - 1;
        }
        return res;
